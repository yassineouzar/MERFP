...
def mask_roi(image):
    p = 'G:/shape_predictor_81_face_landmarks.dat'
    ...
    for i in range(80, 85):
        ...
        for j in range(int(len(list_dir1))):
            ...
            for k in range(int(len(list_dir2))):
                ...
                list_dir_save2 = path_save_im + '/' + list_dir[i] + '/' + list_dir1[j] + '/' + list_dir2[k]
                print(path_to_files1)

                #if not os.path.exists(list_dir_save2):
                    #os.makedirs(list_dir_save2)

                img = cv2.imread(path_to_files1)
                #img = cv2.pyrUp(img)
                #cv2.imshow('img', img)
                #cv2.waitKey(0)
                ...
                cv2.imwrite(list_dir_save2, img)
                count += 1
                print(count)
                #cv2.imshow('img', img)
                #cv2.waitKey(0)
...
path_im = 'G:/V4V/videos/Frames'
path_save_im = 'G:/V4V/videos/Frames'
