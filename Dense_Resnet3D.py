...
def dense_resnet_3d(nb_classes, input_shape, weight_decay=0.01, dropout_rate=0.2):
    ...
    x = MaxPooling3D((1, 2, 2), strides=(1, 5, 5), padding='same')(x)
    # 56x56x8
    ...
    x = add([x, y])
    y = MaxPooling3D((1, 2, 2), strides=(1, 2, 2), padding='same')(x)
    # 28x28x4
    ...
    x= dense_block(y, 32, internal_layers=4,
                   dropout_rate=dropout_rate)
    x = add([x, y])
    x = MaxPooling3D((1, 2, 2), strides=(1, 4, 4), padding='same')(x)
    # 14x14x2
    ...
    x = dense_block(x, 64, internal_layers=4,
                   dropout_rate=dropout_rate)
    x = add([x, y])
    y = MaxPooling3D((1, 2, 2), strides=(1, 5, 5), padding='same')(x)
    # 7x7x1
    ...
    model = Model(inputs=model_input, outputs=x, name="Dense_Resnet3D")
