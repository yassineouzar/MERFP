...
class DirectoryIterator(Iterator):
    ...
    def __init__(self, directory, image_data_generator,
        ...
        for i in range(len(preprocessed_sig) - (len(preprocessed_sig) - len(batches_im))):
            ...
            # print(len(PPGi))
            # for k in range((len(PPGi) - self.frames_per_step) // overlapping):
            for k in range(len(PPGi) // self.frames_per_step):

                # print(k, len(PPGi))
                # batches_ppgi = PPGi[k * overlapping: k * overlapping + self.frames_per_step]
                batches_ppgi = PPGi[k * self.frames_per_step: (k+1) * self.frames_per_step]
                # print(batches_ppgi)
        ...
        for j in batches_im:
            ...
            for k in range(len(j) // self.frames_per_step):
                batch_im = j[k * self.frames_per_step: (k+1) * self.frames_per_step]
    ...
    def next(self):
        ...
        for batch in batches_PPG:
            ...
            indexes_peaks, _ = signal.find_peaks(batch, distance=10)
            # indexes_valleys, _ = signal.find_peaks(-batch, distance=50)
            fps = 25
            time = np.arange(0, ((len(batch) / fps) - 1 // fps), 1 / fps)

            iHR_peaks = np.gradient(time[indexes_peaks])

            rr = np.diff(indexes_peaks)
            ...
            results_2, fxx_2, pxx_2 = frequency_domain(iHR_peaks)
            ...
            for k, v in time_domain_metrics.items():
                # print("- %s: %.2f" % (k, v))
                batch_i.append(round(v, 2))
            batch_y.append(batch_i)
