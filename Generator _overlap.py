...
class DirectoryIterator(Iterator):
    ...
    def __init__(self, directory, image_data_generator,
        ...
        #print(classes)
        #nombres d'image totale dans DB
       ...
       ## cplasstou machi hna 
        ...
        for ti in classes_hr:
            ...
            heart_rate = [filename for filename in list_dir2 if filename.startswith("BP")]
        ...
        for i in range(len(li)-(len(li)-len(batches))):
            ...
            if xx > 0 :
              ...
              #B[len(B):len(B)+xx] = B[len(B)-xx:]
              for test in range(-xx) :
                batches[i].pop()
                #self.filenames.pop()
                #df.drop(df.append(xx).index, inplace=True)
            ...
            #heart_rate = [int(i) for i in heart_rate]
    ...
    def next(self):
        ...
        #---------------------------------TODO----------------------------------------------------
        #---------------------- return PPG signal fragment instead of HR value -------------------
        ...
        for batch_hr in batches55:
            ...
            label1 = int(np.mean(batch_hr))
            batches555.append(label1)

        batch_y = np.array(batches55)
        ...
        for i in range(int(len(index_array))):
            ...
            im = PIL.PIL.split(img)
            x = img_to_array(im[1], data_format=self.data_format)
            #x = x.transpose(1, 0, 2)
        ...
        #batch_x = batch_x1.reshape(-1, self.frames_per_step, 260, 348, 3)
...
train_data = datagen.flow_from_directory(directory='/content/drive/MyDrive/Dataset/ROI', label_dir ='/content/drive/MyDrive/Dataset/HR', target_size=(120, 160), class_mode='label', batch_size=14, frames_per_step=50, shuffle=False)
test_data = datagen.flow_from_directory(directory='/content/drive/MyDrive/Dataset/ROI', label_dir ='/content/drive/MyDrive/Dataset/HR', target_size=(120, 160), class_mode='label', batch_size=14, frames_per_step=50, shuffle=False)
...
if __name__ == '__main__':
    ...
    batch_size=1
    train_data = datagen.flow_from_directory(directory='/MonDrive/Dataset/ROI', label_dir ='/MonDrive/Dataset/HR', target_size=(120, 160), class_mode='label', batch_size=batch_size, frames_per_step=50, shuffle=False)
    print("finished1")
