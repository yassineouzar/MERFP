...
# from Face_landmarks import mpFace
import math
...
def mask_roi(image):
    p = '/home/ouzar1/Documents/pythonProject/shape_predictor_81_face_landmarks.dat'
    detector = dlib.get_frontal_face_detector()
    predictor = dlib.shape_predictor(p)
    pts= np.zeros((30,2), dtype=np.int32)
    ...
    if detector != []:
        ...
        if result != []:
            ...
            for (i, rect) in enumerate(rects):
                ...
                # print(shape)
                # for (k,l) in shape:
                #     cv2.circle(image, (k, l), 1, (0, 255, 25), 1)
                    # cv2.circle(image, (95, 302), 2, (0, 255, 25), 2)
                    # cv2.rectangle(image, (rect.left(), rect.top()), (rect.right(), rect.bottom()), (0, 255, 0), 3)
                pts = np.array(
                    [shape[17], shape[18], shape[19], shape[20], shape[21], shape[22], shape[23], shape[24], shape[25],
                     shape[26], shape[48], shape[49], shape[50], shape[51], shape[52], shape[53], shape[54], shape[55],
                     shape[56], shape[57], shape[58], shape[59], shape[60], shape[61], shape[62], shape[63], shape[64], shape[65], shape[66], shape[67]], np.int32)
                # for (k,l) in pts:
                #     cv2.circle(image, (k, l), 1, (0, 255, 25), 1)
                #
...
class DirectoryIterator(Iterator):
    ...
    def next(self):
        ...
        # The transformation of images is not under thread lock
        # so it can be done in parallel
        batch_x = np.zeros((self.batch_size,) +  (self.frames_per_step,) + (30,) +
                           (2,), dtype=K.floatx())  # # my addition of +(1,)
        batch_y = np.zeros((self.batch_size,), dtype="object")
        batch_x1 = np.zeros((self.frames_per_step * self.batch_size,) +
                            (30,) + (2,), dtype=K.floatx())  # # my addition of +(1,)
        ...
        detector = dlib.get_frontal_face_detector()
        predictor = dlib.shape_predictor(p)
        landmarks = np.zeros((30, 2), dtype=np.int32)
        ...
        for j in range(self.batch_size):
            for i in range(int(len(index_array))):
                ...
                frame = cv2.imread(os.path.join(self.directory, fname))
                ...
                for (i, rect) in enumerate(rects):
                    ...
                    landmarks = np.array(
                                [shape[17], shape[18], shape[19], shape[20], shape[21], shape[22], shape[23], shape[24],
                                 shape[25],
                                 shape[26], shape[48], shape[49], shape[50], shape[51], shape[52], shape[53], shape[54],
                                 shape[55],
                                 shape[56], shape[57], shape[58], shape[59], shape[60], shape[61], shape[62], shape[63],
                                 shape[64], shape[65], shape[66], shape[67]], np.int32)
                    # for (k,l) in landmarks:
                    #     cv2.circle(frame, (k, l), 1, (0, 255, 25), 1)
                # landmarks = mask_roi(frame)
                #
                ...
                # x = np.array(results.multi_face_landmarks)
                # print(results)
                # for xx in x:
                #     print(type(xx))
                ...
                # # for face in faces:
                # #     for lm in face:
                # #         cv2.circle(img,lm,radius,green,-1)
                # for face in faces:
                ...
                batch_x1[i] = landmarks
                #batch_y[j] = self.classes_emo[j]
                #print(self.classes_emo[k])
        #batch_x = batch_x1.reshape(-1, self.frames_per_step, self.image_shape)

        batch_x = batch_x1.reshape((-1,) + (self.frames_per_step,) + (30,) + (2,) )  # # my addition of +(1,)
        # print(os.path.join(self.directory, fname), batch_x.shape)
        ...
        if self.class_mode == 'input':
            ...
            #print(batch_y1)
            #for l in batch_y:
                #for x in np.array(list(self.class_emotions.keys())):
                #print(list(self.class_emotions.keys()==l).all)
                #task = (np.array(list(self.class_emotions.keys()))== l).astype(int)
                #print(np.array(list(self.class_emotions.keys())), l, bool(np.array(list(self.class_emotions.keys()))== l))
                #print(np.vstack((batch_y1,task)))
                #print(np.concatenate(batch_y1, task))


            #batch_y = emo
            #print(batch_y[0], batch_y[3], batch_y[7])
            #for k in range(len(self.classes_emo)//self.frames_per_step):
                #print(k)
            #print(list(self.class_emotions.keys()))
            #for i in list(self.class_emotions.keys()):
                #print(type(i))
            #for i, label in enumerate(self.classes_emo[index_array][1]):
                #print(i, label)
                #batch_y = list(self.class_emotions.keys())[label]
...
if __name__ == '__main__':
    ...
    train_data = datagen.flow_from_directory(directory='/home/ouzar1/Documents/pythonProject/BP4D-Expressive-Segment/multimodal/FE/Test_data',
